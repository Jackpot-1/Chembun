...
func sprint_and_roll():
	...
	if Input.is_action_just_pressed("sprint"):
		if not animations.roll in playback.get_current_node() and not animations.jump in playback.get_current_node() and not animations.bigattack in playback.get_current_node():
			$DashTimer.start()

	if Input.is_action_just_released("sprint") and not $DashTimer.is_stopped():
		$DashTimer.stop()
...
func attack1(): # If not doing other things, start attack1
	if (animations.idle in playback.get_current_node() or animations.walk in playback.get_current_node()) and is_on_floor():
		if Input.is_action_just_pressed("attack") and $"../CanvasLayer".playerStopped == false:
			if (not states.attacking):
...
func _physics_process(delta):
	...
	states.grounded = is_on_floor() # State control for is jumping/falling/landing
	...
	states.attacking = (animations.attack1 in playback.get_current_node() or animations.attack2 in playback.get_current_node() or animations.rollattack in playback.get_current_node() or animations.bigattack in playback.get_current_node())
	...
	if Input.is_action_just_pressed("jump") and not states.attacking and not states.rolling and not playerStopped:
		if states.grounded:
			jump_counter = 0
	...
	if (Input.is_action_pressed("forward") or Input.is_action_pressed("backward") or Input.is_action_pressed("left") or Input.is_action_pressed("right")):
		if(not playerStopped):
			...
			if Input.is_action_pressed("sprint") and $DashTimer.is_stopped() and states.walking:
		...
		states.walking = false
		states.running = false
	if Input.is_action_pressed("aim") and not playerStopped:  # Aim/Strafe input and  mechanics
	...
	if (states.attacking or states.rolling):
	...
	animation_tree["parameters/conditions/IsInAir"] = not states.grounded
	# Moving and running respectively
	animation_tree["parameters/conditions/IsWalking"] = states.walking
	animation_tree["parameters/conditions/IsNotWalking"] = not states.walking
	animation_tree["parameters/conditions/IsRunning"] = states.running
	animation_tree["parameters/conditions/IsNotRunning"] = not states.running
